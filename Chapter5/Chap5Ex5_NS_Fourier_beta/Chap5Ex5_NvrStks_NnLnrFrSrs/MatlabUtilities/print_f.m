...
fprintf(fout,'f_au0\t au0*au0x+av0*au0y-0.5*(%s+%s)\n',s1,s2);
fprintf(fout,'f_atu0\t atu0*atu0x+atv0*atu0y-0.5*(%s+%s)\n',s1,s2);

s1=sprint_da0(n,'u','v','x');
s2=sprint_da0(n,'v','v','y');
fprintf(fout,'f_av0\t au0*av0x+av0*av0y-0.5*(%s+%s)\n',s1,s2);
fprintf(fout,'f_atv0\t atu0*atv0x+atv0*atv0y-0.5*(%s+%s)\n',s1,s2);

for k=1:n
    s1=sprint_dak(k,n,'u','u','x');
    s2=sprint_dak(k,n,'v','u','y');
    fprintf(fout,'f_au%d\t au%d*au%dx+av%d*au%dy-%d*alpha*bu%d-0.5*(%s+%s)*(N>%d)\n',k,k,k,k,k,k,k,s1,s2,k-1);
    fprintf(fout,'f_atu%d\t atu%d*atu%dx+atv%d*atu%dy-%d*alpha*bu%d-0.5*(%s+%s)*(N>%d)\n',k,k,k,k,k,k,k,s1,s2,k-1);
        
    s1=sprint_dak(k,n,'u','v','x');
    s2=sprint_dak(k,n,'v','v','y');
    fprintf(fout,'f_av%d\t au%d*av%dx+av%d*av%dy-%d*alpha*bv%d-0.5*(%s+%s)*(N>%d)\n',k,k,k,k,k,k,k,s1,s2,k-1);
    fprintf(fout,'f_atv%d\t atu%d*atv%dx+atv%d*atv%dy-%d*alpha*bv%d-0.5*(%s+%s)*(N>%d)\n',k,k,k,k,k,k,k,s1,s2,k-1);
    
    s1=sprint_dbk(k,n,'u','u','x');
    s2=sprint_dbk(k,n,'v','u','y');
    fprintf(fout,'f_bu%d\t bu%d*bu%dx+bv%d*bu%dy+%d*alpha*au%d-0.5*(%s+%s)*(N>%d)\n',k,k,k,k,k,k,k,s1,s2,k-1);
    fprintf(fout,'f_btu%d\t btu%d*btu%dx+btv%d*btu%dy+%d*alpha*au%d-0.5*(%s+%s)*(N>%d)\n',k,k,k,k,k,k,k,s1,s2,k-1);
    
    s1=sprint_dbk(k,n,'u','v','x');
    s2=sprint_dbk(k,n,'v','v','y');
    fprintf(fout,'f_bv%d\t bu%d*bv%dx+bv%d*bv%dy+%d*alpha*av%d-0.5*(%s+%s)*(N>%d)\n',k,k,k,k,k,k,k,s1,s2,k-1);
    fprintf(fout,'f_btv%d\t btu%d*btv%dx+btv%d*btv%dy+%d*alpha*av%d-0.5*(%s+%s)*(N>%d)\n',k,k,k,k,k,k,k,s1,s2,k-1);
end
